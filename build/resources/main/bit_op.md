千篇一律的黑客作者：Sean Eron Anderson seander@cs.stanford.edu单独地，
这里的代码段是公共领域的（除非另有说明），请随意使用它们。
总的收集和描述是©1997-2005 Sean Eron Anderson。分发代码和描述是希望它们会有用，
但没有任何保证，甚至没有对适销性或对特定用途适用性的暗示保证。截至2005年5月5日，所有代码均已经过全面测试。
成千上万的人阅读了它。此外，卡内基梅隆大学计算机科学系主任兰德尔·布莱恩特教授已经用他的Uclid代码验证系统亲自测试了几乎所有内容。
他没有测试，我检查了32位计算机上所有可能的输入。
如果第一个通知我代码中的合法错误的人，我将支付10美元（以支票或Paypal）的赏金。如果转交给慈善机构，我将支付20美元。
内容
关于操作计数方法计算整数的符号检测两个整数是否具有相反的符号计算无分支的整数绝对值（abs）
计算无分支的两个整数的最小值（min）或最大值（max）确定是否为整数2的幂
Sign extending
